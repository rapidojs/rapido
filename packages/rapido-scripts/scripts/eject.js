...
const inquirer = require('@rapido/dev-utils/inquirer');
const spawnSync = require('@rapido/dev-utils/crossSpawn').sync;
  ...
  .then(answer => {
    ...
    if (fs.existsSync(paths.appTypeDeclarations)) {
      try {
        ...
        // Remove @rapido/scripts reference since they're getting a copy of the types in their project
        content =
          content
            // Remove @rapido/scripts types
            .replace(
              /^\s*\/\/\/\s*<reference\s+types.+?".*@rapido.*".*\/>.*(?:\n|$)/gm,
              ''
            )
            .trim() + os.EOL;
